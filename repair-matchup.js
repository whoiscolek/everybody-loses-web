...
function buildEmail({ recipient, accepter, event, match, appUrl }) {
  const accepterName = escapeText(accepter?.displayName || "Someone");
  ...
  const pick = escapeText(match?.pick || "their side");
  const url = appUrl || "https://everybody-loses.vercel.app";

  return {
    to: recipient.email,
    subject: `${accepterName} matched your bet on ${eventTitle}`,
    html: `
      <div style="font-family:Inter,Arial,sans-serif;line-height:1.45;color:#111827;max-width:560px;margin:0 auto;padding:24px;">
        <h2 style="margin:0 0 12px;letter-spacing:-.03em;">Your bet got matched 👎</h2>
        <p style="margin:0 0 16px;color:#374151;">${accepterName} accepted/matched your open bet.</p>
        <div style="border:1px solid #e5e7eb;border-radius:16px;padding:16px;background:#f9fafb;">
          ...
          <p style="margin:12px 0 0;color:#111827;"><strong>${amount}</strong> matched against <strong>${pick}</strong></p>
        </div>
    ...
    `,
    text: `${accepterName} matched your ${amount} bet on ${eventTitle} (${league}, ${time}). Open Everyone Loses: ${url}`
  };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { recipient = {}, accepter = {}, event = {}, match = {} } = req.body || {};
    ...
    return json(res, 200, { sent: 1, failed: 0, id: result?.id || "" });
  } catch (error) {
    return json(res, error.status || 500, { error: error.message || "Failed to send matchup notification." });
  }
}
